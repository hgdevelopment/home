...
<div class="bg-light lter b-b wrapper-md">
  <h1 class="m-n font-normal h3">TCN {{ $withDraws->type }} Approval</h1>
</div>
...
<div class="wrapper-md" ng-controller="FormDemoCtrl">
	<form role="form" method="post" id="form"  action="{{URL::to('/') }}/admin/tcnFullWithDrawal/{{ $withDraws->id }}" data-parsley-validate >
		{{ csrf_field() }}
		{{ method_field('PUT') }}
...
<div class="col-lg-7">
		...
		<div class="panel-body">
		  ...
		  <div class="col-md-6 col-xs-12 divTableCell">Withdrawal Amount<b class="data-lab">{{$withDraws->amount}}</b></div> 
		</div>
		<br>
		<div class="panel-body">
		<div class="row">
...
{{-- 							<select class="chosen-select" name="currencyType" id="currencyType" required data-parsley-required-message="Please Select currencyType">
 ...
 --}}
							<input type="text" name="currencyType" id="currencyType" value="{{ $tcnrequests->currencyType }}" class="form-control" readonly />
				...
				<div class="col-sm-12">
					<div class="row">
						...
						<div class="col-sm-3">
	                      <input type="text" class="form-control"  id="wdappliedDate" name="wdappliedDate" value="{{date('d-m-Y',strtotime($withDraws->appliedDate))}}" required data-parsley-required-message="Please Enter Withdrawal applied date" readonly>					
						</div>
						<div class="col-sm-3 font-bold">Withdrawal paid date<span class="text-danger">*</span></div>
						<div class="col-sm-3">
	                      <input type="text" data-date-end-date="0d" class="form-control" id="wdpayedDate" name="wdpayedDate" value="{{date('d-m-Y')}}" required data-parsley-required-message="Please Enter Withdrawal paid date" readonly>
						</div>
					</div>
				</div><div class="col-sm-12">&nbsp;</div>
				...
				<div class="col-sm-12">
					<div class="row">
						...
						<div class="col-sm-3">
							<input type="text" name="debitAccountNo" id="debitAccountNo" value="{{ $tcnrequests->heeraaccount->accountNumber }}" class="form-control" required data-parsley-required-message="Please Enter Debit acc No.(Heera's)" onblur="heeraAccountCheck(this.value)" readonly />
						</div>
						<div class="col-sm-3 font-bold">Bank</div>
						<div class="col-sm-3">
							<input type="text" name="bank" id="bank" value="{{ $tcnrequests->heeraaccount->bankName }}" class="form-control" readonly />
						</div>
					</div>
				</div><div class="col-sm-12">&nbsp;</div>
				...
				<input type="hidden" name="tcnId" id="tcnId" value="{{$tcnrequests->id}}"> 
				<input type="hidden" name="withDrawId" id="withDrawId" value="{{$withDraws->id}}"> 
...
if(heeraAccountNumber=="")
	...
	$.ajax({
	type: "get",
    url: "{{URL::to('/') }}/admin/normalWithDraw/create",
