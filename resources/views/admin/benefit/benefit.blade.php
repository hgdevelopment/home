...
<a href="{{URL::to('/')}}/admin/benefit" class="btn btn-dark" id="back" style="float: left;">Back</a>
...
<p id="statusajax{{$tcnmasters->id}}" style="padding: 0px 0px 2px 15px;text-align: right;"></p>
...
@foreach($tcnmaster as $tcnmasters)
  $('#month{{$tcnmasters->id}}').change(function(event) {
   ...
   $.ajax({
            type: "get",
            url: "{{URL::to('/')}}/admin/benefit/check",
            data: data,
            cache: false,
            success: function(result){
               if(result != 0){
                ...
                if(result.status == '2' || result.status == '3'){
                     ...
                     $( '#statusajax{{$tcnmasters->id}}' ).html( "<span style='color:red;'>Benefit Generated already</span>" );
...
$('#update{{$tcnmasters->id}}').click(function(event) {
   ...
   var a = $('#inr{{$tcnmasters->id}}').val();
   var b = $('#aed{{$tcnmasters->id}}').val();
   var c = $('#sar{{$tcnmasters->id}}').val();
   var d = $('#usd{{$tcnmasters->id}}').val();
   var e = $('#cad{{$tcnmasters->id}}').val(); 
   
   if(a > 0){$('#inrnote{{$tcnmasters->id}}').html('');}else{$('#inrnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter INR amount</span>');$('#update{{$tcnmasters->id}}').removeClass('animated bounce infinite');return;}
    if(b > 0){$('#aednote{{$tcnmasters->id}}').html(''); }else{$('#aednote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter AED amount</span>');$('#update{{$tcnmasters->id}}').removeClass('animated bounce infinite');return;}
     if(c  > 0){ $('#sarnote{{$tcnmasters->id}}').html('');}else{$('#sarnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter SAR amount</span>');$('#update{{$tcnmasters->id}}').removeClass('animated bounce infinite');return;}
      if(d  > 0){ $('#usdnote{{$tcnmasters->id}}').html('');}else{$('#usdnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter USD amount</span>');$('#update{{$tcnmasters->id}}').removeClass('animated bounce infinite');return;}
       if(e  > 0){$('#cadnote{{$tcnmasters->id}}').html(''); }else{$('#cadnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter CAD amount</span>');$('#update{{$tcnmasters->id}}').removeClass('animated bounce infinite');return;}

   var data = $('#form{{$tcnmasters->id}}').serialize();
   $.ajax({
            type: "post",
            url: "{{URL::to('/')}}/admin/benefit/add",
            data: data,
            cache: false,
            success: function(result){
               if(result != 0){
...
$('#submit{{$tcnmasters->id}}').click(function(event) {
   ...
   if(a > 0){$('#inrnote{{$tcnmasters->id}}').html('');}else{$('#inrnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter INR amount</span>');return;}
    if(b > 0){$('#aednote{{$tcnmasters->id}}').html(''); }else{$('#aednote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter AED amount</span>');return;}
     if(c  > 0){ $('#sarnote{{$tcnmasters->id}}').html('');}else{$('#sarnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter SAR amount</span>');return;}
      if(d  > 0){ $('#usdnote{{$tcnmasters->id}}').html('');}else{$('#usdnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter USD amount</span>');return;}
       if(e  > 0){$('#cadnote{{$tcnmasters->id}}').html(''); }else{$('#cadnote{{$tcnmasters->id}}').html('<span style="color:red;">Please enter CAD amount</span>');return;}
