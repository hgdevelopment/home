...
<div class="bg-light lter b-b wrapper-md">
	...
	<br>
	<form action="{{ URL::to('/') }}/hr/resignation/view" method="post" enctype="multipart/form-data" data-parsley-validate >
	{{csrf_field()}}
		<div class="row">
			<div class="col-sm-12">
				<div class="blog-post">                   
									...
									<table class="table table-striped table-bordered dt-responsive nowrap b-t" cellspacing="0" width="100%"  id="enquiryReport">
										...
										@foreach ($report as $reports)
											<tr>
												...
												<td>{{$reports->resign_date}}</td>
												<td>{{$reports->resign_status}}</td>
												<td>{{$reports->status_date}}</td>
											</tr>
										@endforeach
										</tbody>
									</table>
