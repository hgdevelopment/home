...
@endif
@if($data->employeeType=="ME")
<script>
$(function(){
  var table = $('#users-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{URL::to('/') }}/admin/report_incentive_inner_me/{{$id}}',
        columns: [
  ...
  $(document).on('click','.report-btn-view',function(e){
     e.preventDefault();
    $.post('{{URL::to('/') }}/admin/report_incentive_me',{
