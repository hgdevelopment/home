...
@if($request->opcode==7)
	...
	@endphp
		...
		<input class="form-control" type="text"  data-date-end-date="0d{{ $n }}" id="dob{{ $n }}" name="dob{{ $n }}" value="@if(isset($nominee->dob)){{date('m-d-Y',strtotime($nominee->dob))}}@endif" required data-parsley-required-message="Please Select Correct DOB" readonly />
		...
		<select class="form-control col-md-6" name="typeOfProof{{ $n }}" id="typeOfProof{{ $n }}" >
		<option value=" ">Select</option>
		...
		<img  alt="uploadPhoto" style="width:200px;height:200px;" src="{{ URL::to('/') }}/storage/img/nominee/{{$nominee->uploadPhoto}}" onclick="window.open('{{ URL::to('/') }}/storage/img/nominee/{{$nominee->uploadPhoto}}')" download >
		</div>
		@endif

		@if(isset($nominee->signature))
		<div class="col-sm-4 nomineePhoto{{ $n }}"><br>
		<img  alt="proofId" style="width:200px;height:200px;" src="{{ URL::to('/') }}/storage/img/nominee/{{$nominee->signature}}" onclick="window.open('{{ URL::to('/') }}/storage/img/nominee/{{$nominee->signature}}')" download>
		</div>
		@endif
		...
		@if(isset($proof->file))
		<div class="col-sm-4 nomineePhoto{{ $n }}"><br>
		<img  alt="Proof Copy" style="width:200px;height:200px;" src="{{ URL::to('/') }}/storage/img/proof/{{$proof->file}}" onclick="window.open('{{ URL::to('/') }}/storage/img/proof/{{$proof->file}}')" download>
		</div>
		@endif
		</div>
		</div>
...
<td>
<div class="text-center" style="color:black;background:@if($detail->docVerified=='Pending'){{'yellow'}}@elseif($detail->docVerified=='Verified'){{'#63ce63'}}@endif">
{{ $detail->docVerified }}
</div>
...
<li>
<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{'view@@@'}}{{encrypt($detail->tcnId)}}">View
</a>
</li>

@if($detail->status=='Approved' && $detail->status!='Removed')
<li>
<a href="{{ URL::to('/') }}/admin/tcnviewprint/{{encrypt($detail->tcnId)}}" >Print
</a>
</li>
@endif
...
<li>
<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($detail->tcnId)}}">Document Verification
</a>
</li>
@endif
...
<li>
<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($detail->tcnId)}}">Payment Verification
</a>
</li>
@endif
...
<li>
<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($detail->tcnId)}}">Reapproval
</a>
</li>
@endif	
...
<li>
<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($detail->tcnId)}}/edit">Edit
</a>
</li>
@endif
