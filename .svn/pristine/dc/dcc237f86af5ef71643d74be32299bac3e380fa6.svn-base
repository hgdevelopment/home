...
class tcnTransferController extends Controller
{
    ...
    $newTcnReq->transferredReason='This TCN is Transferred from old TCN Code : '.$oldTcn->tcnCode.'-Member Code '.$oldMember->code;
    $newTcnReq->save();
    ...
    DB::table('tcnrequests')
        ...
        ->update([
            ...
            'transferredReason' =>$remarks.'.. This TCN is transferred to new TCN Code : TCN'.$tcnNo.'-Member Code '.$newMember->code]);
...
// return var_dump($data);
    ...
    $type='TCN';
    $report='TCN Transferred .  From : '.$oldTcn->tcnCode.' - To : TCN'.$tcnNo;
