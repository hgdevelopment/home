...
//        dd(\DB::getQueryLog());
    ...
    {
        $benefitdeclarations =benefitdeclaration::where([['tcnType',$request->tcnType],['status','1']])->get();
...
//      echo $month.'--<br>'.$from.'<br>'.$to;
        ...
        $tcns = tcnrequest::with('member')->where([['tcn_id',$tcnid],['status','Approved'],['physicalBenefit','yes']])->where('depositeDate','<',$to)->get();
        $count = count($tcns);

        if($count == '0'){
            Session::flash('message', 'No Tcn Request available for Benefit ('.$month1->format("F").'/'.$month1->format("Y").')'); 
            return redirect('admin/benefit/view');
        }
        ...
        while ($i<$count){
            ...
            $check = login::find($tcns[$i]->userId);
        ...
        if($countCheck != '1'){
            ...
            switch ($ctype) {                                 
                case 'INR': 
                $benefit->memberFee = ($tcns[$i]->unit)*50;   break;
                case 'AED': 
                $benefit->memberFee = ($tcns[$i]->unit)*4;   break;
                case 'SAR': 
                $benefit->memberFee = ($tcns[$i]->unit)*4;   break;
                case 'USD': 
                $benefit->memberFee = ($tcns[$i]->unit)*1;   break;
                case 'CAD': 
                $benefit->memberFee = ($tcns[$i]->unit)*1;   break;
            ...
            $benefit->total = ($share->$ctype)*($tcns[$i]->unit)-$benefit->memberFee;
    ...
    if($request->type == 'All'){
        ...
        $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['benefitType','NORMAL']])->get();
        ...
        if($request->type == 'bankinr'){
            ...
            $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['currencyType','INR'],['benefitType','NORMAL']])->groupBy('bankAccountId')->selectRaw('sum(total) as sum, userId, userName,bankAccountId')->get();
        ...
        if($request->type == 'bankaed'){
            ...
            $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['currencyType','AED'],['benefitType','NORMAL']])->groupBy('bankAccountId')->selectRaw('sum(total) as sum, userId, userName,bankAccountId')->get();
        ...
        if($request->type == 'banksar'){
            ...
            $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['currencyType','SAR'],['benefitType','NORMAL']])->groupBy('bankAccountId')->selectRaw('sum(total) as sum, userId, userName,bankAccountId')->get(); 
        ...
        if($request->type == 'bankcad'){
            ...
            $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['currencyType','CAD'],['benefitType','NORMAL']])->groupBy('bankAccountId')->selectRaw('sum(total) as sum, userId, userName,bankAccountId')->get();
...
if($request->type == 'bankusd'){
            ...
            $banks = benefitgeneration::where([['year',$x[0]],['month',$x[1]],['tcnType',$request->tcnId],['currencyType','USD'],['benefitType','NORMAL']])->groupBy('bankAccountId')->selectRaw('sum(total) as sum, userId, userName,bankAccountId')->get();
