				                ...
				                <td>
									<div class="btn-group dropdown">
								      ...
								      <ul class="dropdown-menu dropdown-menu-right" >
								        <li>
									        <a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{'view@@@'}}{{encrypt($tcnrequest->tcnId)}}">View
											</a>
										</li>

										@if($tcnrequest->status=='Approved' && $tcnrequest->status!='Removed')
								        <li>
									        <a href="{{ URL::to('/') }}/admin/tcnviewprint/{{encrypt($tcnrequest->tcnId)}}" >Print
											</a>
										</li>
										@endif
								      ...
								      @if($tcnrequest->docVerified=='Pending' && $tcnrequest->status=='Pending' && Controller::UserAccessRights('TCN Document Verification')=='1' && $tcnrequest->status!='Removed')
									        <li>
										        <a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($tcnrequest->tcnId)}}">Document Verification
												</a>
											</li>
									  @endif
								      ...
								      @if($tcnrequest->paymentReceived=='Pending' && $tcnrequest->docVerified!='Pending'  && $tcnrequest->status=='Pending' && Controller::UserAccessRights('TCN Payment Verification')=='1' && $tcnrequest->status!='Removed') 
								        <li>
									        <a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($tcnrequest->tcnId)}}">Payment Verification
											</a>
										</li>
									  @endif
								      ...
								      @if($tcnrequest->status=='Denied' && Controller::UserAccessRights('TCN Reapproval')=='1')
								        <li>
									        <a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($tcnrequest->tcnId)}}">Reapproval
											</a>
										</li>
									  @endif	
										...
										<li>
										<a href="{{ URL::to('/') }}/admin/tcnApplicationForm/{{encrypt($tcnrequest->tcnId)}}/edit">Edit
										</a>
										</li>
										@endif
