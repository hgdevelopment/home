...
{{-- 					  	@php
						$uniqmid=DB::table('memberregistrations')
						->whereExists(
							function ($query)
							 {
							 	$query->select(DB::raw(1))->from('tcnrequests')->whereRaw('tcnrequests.userId = memberregistrations.userId')
							 	->whereExists(
								function ($query)
								 {
								 	$query->select(DB::raw(1))->from('logins')->whereRaw('logins.id = memberregistrations.userId')->where('logins.status','Active');
								 });
								 })
						->get();
				  		@endphp
 --}}					  <select class="form-control col-md-6 chosen-select" name="userId" id="userId">
			...
			<div class="col-sm-4" style="border-color: #cfdadd;border-radius: 2px;border:1px solid #ccc;">
				<div class="col-sm-12" style=" padding:0 3px;"><span style="font-size: 12px; color: blue">( Here you can see the transferred TCN also .. )</span>
